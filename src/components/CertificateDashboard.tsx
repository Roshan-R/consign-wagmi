...
const CertificateDashboard = ({ title, to_addr, image, num_approvals, addrs }) => {
    ...
    return (
        ...
        <div className="border-brutal bg-white flex flex-col">
            ...
            <div className="border-brutal flex grow justify-center bg-purple-400">
                <img src={image} onClick={(e) => { setShowModal(true) }} className="border-brutal m-2 w-4/5 bg-gray-200 cursor-pointer hover:opacity-90 transition-all"></img>
            </div>

            {showModal ? <div className="z-10 h-screen w-screen fixed inset-0 flex justify-center">
                <img src={image} className="border-brutal object-contain ">
                </img>
                <button onClick={(e) => { setShowModal(false) }} className="absolute bg-peachh font-roboto bottom-2 border-brutal p-1">close</button>
            </div> : <></>}
